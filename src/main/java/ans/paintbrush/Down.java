...
public class Down extends Shape {
...
//    private Color Color;
    ...
    public Down(int x1, int y1, int x2, int y2) {
        super(x1, y1, x2, y2);
    }
 ...
 @Override
    public String toSVG() {
     ...
     public static Shape loadSVG(org.w3c.dom.Node node){
        ...
        down.getP1().x = Integer.parseInt(values[2]);
        down.getP1().y = Integer.parseInt(values[7]);
        down.getP2().x = Integer.parseInt(values[12]);
        down.getP2().y = Integer.parseInt(values[1]);
        down.setDrawColor(Color.decode(stroke.getNodeValue()));
        return down;
    }
}